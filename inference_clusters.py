...
# CONFIG
###################################################################################################
def load_configs(config_path="./configs/config_INFERENCE_v4.json"):
    """
...
def load_models(args, device):
    ...
    checkpoint_controlnet = torch.load(
        args.trained_controlnet_path, map_location=device
    )
...
def create_label_tensor(label):
    tensor = torch.zeros(1, 16, 256, 256)
    tensor[0, label, :, :] = 1.0
    return tensor


def sample_image_unet(timesteps, device, diffusion_unet, latents, noise_scheduler):
    for t in timesteps:
...
def generate_images_in_batches(
    ...
    noise_latents = (
        initialize_noise_latents(1, latent_shape, device) * 1.0
    )  # noise_factor = 1.0

    # Generate images in batches
    with tqdm(total=total_images, desc="Generating images") as pbar:
        for image_idx in range(total_images):
            latents = noise_latents.clone()
            ...
            with torch.no_grad():
                ...
                if image_idx >= 16:
                    ...
                    controlnet_cond = create_label_tensor(image_idx).to(device)
                    latents = sample_image_controlnet(
                        timesteps=timesteps,
            ...
            if (image_idx + 1) % args.checkpoint_interval == 0:
                checkpoint_path = os.path.join(
                    output_dir, f"generation_checkpoint_{image_idx}.json"
                )
                with open(checkpoint_path, "w") as f:
                    json.dump(
                        {
                            "completed_images": image_idx + 1,
                        },
                        f,
                    )
